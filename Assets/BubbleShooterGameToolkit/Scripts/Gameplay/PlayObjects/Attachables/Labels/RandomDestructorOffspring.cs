...
namespace BubbleShooterGameToolkit.Scripts.Gameplay.PlayObjects.Attachables.Labels
{
    public class RandomDestructorOffspring : MonoBehaviour
    {
        ...
        {
            animScale = transform.localScale;
        ...
        void Update()
        {
            if (targetBall != null)
            {
                ...
                // adding arc motion
                newPosition.y += Mathf.Sin(fracJourney * Mathf.PI) * 0.2f;

                // update the object's position
                transform.position = newPosition;
                transform.localScale = Vector3.Lerp(transform.localScale, animScale*.5f, Time.deltaTime * 2f);

                if(fracJourney >= 10f && !hasCheckedAtHalfway)
                {
            ...
            else
            {
                ...
                transform.localScale = Vector3.Lerp(transform.localScale, animScale*1.5f, Time.deltaTime * 2f);
            }
